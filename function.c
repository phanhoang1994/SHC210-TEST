...
*/
	...
	if(GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_0) != 0){Mode_Display=1;}else{Mode_Display=0;}
	
	
	if (Mode_Display==1){if(GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_1) != 0){ pvScroll=vScrollV;} else { pvScroll=vScrollH;}}
	else{pvScroll=vScrollV;}
